...
import { ContentReqInterface, ContentResInterface, CourseResInterface } from "../../utils/interfaces";
    ...
    try {
        ...
        const createdCourse = await prisma.$transaction(async (prisma) => {
            ...
            await prisma.courses.update({
                ...
                data: {
                    instructors: { connect: req.body.instructors.map((inst: string) => (
                        { id: inst }
                    )) }
                }
            })
...
export const addContent = async (req: Request, res: Response) => {
    try {
        ...
        if(existedContent) {
            return writeJsonRes<null>(res, 400, null, "This content name is already used!");
        }
        ...
        return writeJsonRes<ContentResInterface>(res, 201, content, "Successfully created!");
    } catch (error) {
...
export const addInstructor = async (req: Request, res: Response) => {
    try {
        ...
        if(existedInstructor) {
            return writeJsonRes<null>(res, 400, null, "This instructor is already exist!");
        }
